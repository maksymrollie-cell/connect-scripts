...
.DESCRIPTION
    ...
    and renames files and directories whose names contain invalid Unicode
    characters — unpaired surrogates (U+D800-U+DFFF) and the Unicode
    replacement character (U+FFFD) — replacing them with "_".
...
$APPLY   = $true    # change to $true to apply renames
$LOG_DIR = $env:TEMP # or set explicit path e.g. "C:\Logs"

Write-Host "$(Get-Date -Format 'MM/dd/yyyy HH:mm:ss'): Script started on $env:COMPUTERNAME"

# ── Validate required parameters ──────────────────────────────────────────────

if (-not $MC_URL) {
    Write-Host "ERROR: TAG_MC_URL is not set. Set it as an Agent Tag in MC." -ForegroundColor Red
    Write-Host "$(Get-Date -Format 'MM/dd/yyyy HH:mm:ss'): Script finished on $env:COMPUTERNAME"
    exit 1
}
if (-not $TOKEN) {
...
# ── Invalid-character detection & name sanitization ───────────────────────────
#
# We treat as "invalid" any character that is:
#   - an unpaired UTF-16 surrogate (U+D800-U+DFFF), or
#   - the Unicode replacement character (U+FFFD), which Windows shows when a
#     byte sequence couldn't be decoded into a real Unicode codepoint.
#
# We don't try to diagnose *why* a character is broken — anything matching
# these two cases gets collapsed into a single "_".

function Test-IsInvalidCodepoint {
    param([int]$Codepoint)
    return (($Codepoint -ge 0xD800 -and $Codepoint -le 0xDFFF) -or $Codepoint -eq 0xFFFD)
}

function Test-HasInvalidChar {
    param([string]$Name)
    foreach ($ch in $Name.ToCharArray()) {
        $cp = [int][char]$ch
        if (Test-IsInvalidCodepoint $cp) { return $true }
    }
    return $false
}

function Get-InvalidCharPositions {
    param([string]$Name)
    $positions = @()
    $chars = $Name.ToCharArray()
    for ($i = 0; $i -lt $chars.Length; $i++) {
        $cp = [int][char]$chars[$i]
        if (Test-IsInvalidCodepoint $cp) {
...
function Get-CleanedSegment {
    # Collapses any run of invalid codepoints into a single REPLACEMENT_CHAR.
...
function Get-SanitizedName {
    ...
    # NOTE: [System.IO.Path]::GetExtension/GetFileNameWithoutExtension split
    # on the LAST '.' in the name. If the invalid character happens to land
    # inside what .NET considers the "extension" (e.g. "file.xl<bad>sx"),
    # that part is NOT covered by GetFileNameWithoutExtension — so we must
    # sanitize the extension too, not just the base name.

    $ext  = [System.IO.Path]::GetExtension($Name)
    $base = [System.IO.Path]::GetFileNameWithoutExtension($Name)

    $cleanedBase = Get-CleanedSegment $base
    $cleanedExt  = Get-CleanedSegment $ext

    $cleanedBase = $cleanedBase.Trim($REPLACEMENT_CHAR)
    if (-not $cleanedBase) { $cleanedBase = "renamed" }

    # Keep the leading dot of the extension if Get-CleanedSegment stripped it
    if ($cleanedExt -and -not $cleanedExt.StartsWith(".")) {
        $cleanedExt = "." + $cleanedExt.TrimStart($REPLACEMENT_CHAR)
    }
...
# Collect all rename operations for the final summary block (deduplicated)
$renameLog    = [System.Collections.Generic.List[string]]::new()
$renameLogSet = [System.Collections.Generic.HashSet[string]]::new()
...
function Invoke-Rename {
    ...
    $surInfo  = if ($SurrogateInfo) { " invalid_chars: $($SurrogateInfo -join ', ')" } else { "" }
    ...
    if ($Apply) {
        ...
        if ($ok) {
            ...
            if ($renameLogSet.Add($msg)) { $renameLog.Add($msg) }
            return $true
        } else {
...
function Repair-Directory {
    ...
    foreach ($entry in $sorted) {
        ...
        if (-not (Test-HasInvalidChar $name)) {
            Write-Log "  [OK]     '$name'" 
            $stats.Clean++
            continue
        }

        # Get invalid-character details for log
        $invalidChars = Get-InvalidCharPositions $name
...
function Get-DirsToScan {
    ...
    # Collect-Entries (used by Repair-Directory) recurses into subfolders
    # automatically. If one candidate directory is itself inside another
    # candidate directory, processing both would scan/rename the same files
    # twice (and "rename" an already-fixed file again). Keep only the
    # top-most directories.
    $all = @($dirs | Sort-Object)
    $top = [System.Collections.Generic.List[string]]::new()
    foreach ($d in $all) {
        $isNested = $false
        foreach ($other in $all) {
            if ($other -ne $d -and $d.StartsWith($other + [System.IO.Path]::DirectorySeparatorChar)) {
                $isNested = $true
                break
            }
        }
        if (-not $isNested) { $top.Add($d) }
    }
    return @($top)
}
